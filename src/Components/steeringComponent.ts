...
// --------------------------------------------------------------------------------------------------
//    CODE COPIED  FROM https://github.com/dodocloud/games-course/tree/master/examples/src/05-dynamics
//    CHANGED TO WORKED WITH matter js
// --------------------------------------------------------------------------------------------------
const ATTR_VELOCITY = 'velocity';
const ATTR_ACCELERATION = 'acceleration';
const ATTR_BINDER = 'binder';
...
export abstract class SteeringComponentBase<T extends SteeringComponentProps> extends ECS.Component<T> {
	...
	onInit() {
        ...
        this.owner.assignAttribute(ATTR_BINDER,this.props.binder)
	}
	...
	onUpdate(delta: number, absolute: number) {
		...
		Matter.Body.rotate(ownerMatter.body,currentAngle)
	...
	protected applyVelocity(delta: number, gameSpeed: number) {
		...
		const ownerMatter = this.owner as PixiMatter.MatterBody
		Matter.Body.setVelocity(ownerMatter.body,this.velocity)
	}
	...
	protected applyPosition(delta: number, gameSpeed: number) {
		...
		// CHANGED
		const ownerMatter = this.owner as PixiMatter.MatterBody
		Matter.Body.setPosition(ownerMatter.body, deltaPos.add(new ECS.Vector(ownerMatter.position.x,ownerMatter.position.y)))
